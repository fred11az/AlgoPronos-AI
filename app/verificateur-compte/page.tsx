...
// ─── Étapes de simulation de connexion ────────────────────────────────────
...
const AFFILIATE_URL = process.env.NEXT_PUBLIC_1XBET_AFFILIATE_URL ||
...
export default function VerificateurPage() {
  ...
  async function handleVerify() {
    ...
    // Lancer la vraie vérification en parallèle (silencieuse)
    ...
    // Jouer la simulation étape par étape
    let elapsed = 0;
    ...
    <main className="min-h-screen bg-background flex flex-col items-center justify-start pt-16 pb-20 px-4">
      ...
      <div className="text-center mb-10 max-w-xl">
        <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-full px-4 py-2 mb-5">
          ...
          <span className="text-primary text-sm font-semibold">Vérificateur AlgoPronos</span>
        </div>
                    ...
                    <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-surface border border-surface-light rounded-xl shadow-lg overflow-hidden">
                      {BOOKMAKERS.map(bm => (
                        ...
                        >
                          ...
                          {bm.id === '1xbet' && (
                            <span className="ml-auto text-xs bg-primary/20 text-primary px-2 py-0.5 rounded-full">Recommandé</span>
                          )}
                        </button>
                      ))}
                    </div>
              ...
              {optimized ? (
                ...
                <div className="text-center">
                  ...
                  <div className="inline-block bg-success/10 border border-success/20 rounded-xl px-5 py-3 mb-4">
                    <p className="text-success text-xl font-bold tracking-wide">
                      COMPTE OPTIMISÉ IA ✅
                    </p>
                  ...
                  {reason !== 'pending_review' && (
                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 mb-5 text-left">
                      ...
                      <ul className="text-text-secondary text-sm space-y-1.5">
                        <li className="flex items-start gap-2"><span className="text-primary mt-0.5">→</span> Les paramètres de profil bookmaker impactent les cotes proposées</li>
                        <li className="flex items-start gap-2"><span className="text-primary mt-0.5">→</span> Un compte créé via AlgoPronos est configuré pour maximiser les gains</li>
                        <li className="flex items-start gap-2"><span className="text-primary mt-0.5">→</span> Accès illimité à l&apos;algorithme IA une fois validé</li>
                      </ul>
                    </div>
                  )}
                  <div className="flex flex-col gap-2">
